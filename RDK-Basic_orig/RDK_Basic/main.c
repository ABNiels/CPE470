...
void prntBtnSt(BYTE status) {
	...
	DelayMs(4);
    char buffer[20];
    int n = sprintf(buffer, "State of Btn 1:");
    SpiPutBuff(buffer, n);
    
	SpiPutBuff(szCursorPos, 6);
	DelayMs(4);
    n = sprintf(buffer, "%s", (status == stPressed)? "Pressed" : "Released");  
    SpiPutBuff(buffer, n);
    DelayMs(4);
    SpiDisable();
...
void __ISR(_TIMER_5_VECTOR, ipl7) Timer5Handler(void)
{
    ...
    if (btnBtn1.stPrev != btnBtn1.stCur)
        prntBtnSt(btnBtn1.stCur);
